...
int main ()
{
  ...
  **/
  ...
  pid_throttle.Init(Throt_Kp, Throt_Ki, Throt_Kd, Throt_output_lim_max, Throt_output_lim_min);
        ...
        if (s != "") {
          ...
          **/
          ...
          for (int i = 1; i < x_points.size(); ++i) {
                ...
                double distance = sqrt(pow((x_points[i] - x_position), 2) +
                                       pow((y_points[i]) - y_position, 2));
